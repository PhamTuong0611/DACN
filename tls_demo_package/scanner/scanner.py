...
def scan(
    ...
    export_format: Optional[str] = typer.Option(
        ...
        "--export",
        "-e",
        help="Export format: json,csv,html,markdown (comma-separated)"
    ),
...
def serve(
    ...
    async def handle_export(request: web.Request) -> web.Response:
        ...
        reader = await request.post()
        formats = reader.getall("formats") if "formats" in reader else ["json", "csv", "html", "markdown"]
        ...
        try:
            ...
            # Create a success message with download links
            links_html = "<br>".join([
