...
def check_security_headers(headers: Dict[str, str]) -> List[Dict[str, str]]:
    """Kiểm tra tất cả security headers - phân loại theo tầm quan trọng.
    
    Xử lý case-insensitive header names vì HTTP headers không phân biệt hoa thường.
    """
    findings: List[Dict[str, str]] = []
    
    # Create lowercase header map for case-insensitive lookup
    headers_lower = {key.lower(): (key, value) for key, value in headers.items()}
    
    # Check critical headers
    for header_name, header_info in CRITICAL_SECURITY_HEADERS.items():
    ...
    for header_name, header_info in IMPORTANT_SECURITY_HEADERS.items():
        header_lower = header_name.lower()
        if header_lower not in headers_lower:
            findings.append({
                "rule": f"{header_name.upper()}_MISSING",
    ...
    for header_name, header_info in OPTIONAL_SECURITY_HEADERS.items():
        header_lower = header_name.lower()
        if header_lower not in headers_lower:
            findings.append({
                "rule": f"{header_name.upper()}_MISSING",
...
def check_cookie_attributes(set_cookie: str) -> List[Dict[str, str]]:
    ...
    Xử lý case-insensitive: secure, httponly, samesite có thể viết hoa/thường
    """
    findings: List[Dict[str, str]] = []
    ...
    for attr, info in required_attrs.items():
        ...
        if attr_key not in cookie_lower:
            ...
            if attr == "SameSite":
                # Check SameSite value - None is weak for third-party cookies
                if "samesite=strict" in cookie_lower or "samesite=lax" in cookie_lower:
                    findings.append({
                        "rule": f"COOKIE_{attr.upper()}_VALID",
                        "severity": "INFO",
                        "detail": f"Cookie has valid {attr}",
                    })
                elif "samesite=none" in cookie_lower:
                    # SameSite=None only ok if Secure flag present
                    if "secure" in cookie_lower:
                        findings.append({
                            "rule": f"COOKIE_{attr.upper()}_NONE_SECURE",
                            "severity": "LOW",
                            "detail": f"Cookie has SameSite=None with Secure flag (acceptable for third-party)",
                        })
                    else:
                        findings.append({
                            "rule": f"COOKIE_{attr.upper()}_NONE_INSECURE",
                            "severity": "MEDIUM",
                            "detail": f"Cookie has SameSite=None without Secure flag (risky)",
                        })
                else:
                    findings.append({
                        "rule": f"COOKIE_{attr.upper()}_WEAK",
                        "severity": "LOW",
                        "detail": f"Cookie {attr} should be Strict or Lax, not None",
                    })
            else:
                findings.append({
...
def score_findings(findings: List[Dict[str, str]]) -> int:
    """Tính điểm dựa trên mức độ nghiêm trọng của từng phát hiện.
    
    Chỉ tính các issue CRITICAL, HIGH, MEDIUM, LOW
    Bỏ qua INFO (advisory only)
    """
    score = 0
...
def classify_risk(score: int) -> str:
    """Phân loại ngưỡng rủi ko dựa trên điểm số.
    
    Scoring:
    - CRITICAL: 20 pts (e.g., weak TLS, expired cert)
    - HIGH: 10 pts (e.g., missing HSTS, weak cipher)
    - MEDIUM: 5 pts (e.g., missing content-type header)
    - LOW: 1 pt (e.g., missing optional headers)
    - INFO: 0 pts
    
    Risk thresholds:
    - CRITICAL: >=40 (2+ HIGH or 1 CRITICAL + others)
    - HIGH: >=20 (2+ HIGH issues)
    - MEDIUM: >=6 (1 MEDIUM issue or 2+ MEDIUM issues)
    - LOW: >=1 (only LOW or INFO issues)
    - INFO: 0 (no issues)
    """
    if score >= 40:
        return "CRITICAL"
